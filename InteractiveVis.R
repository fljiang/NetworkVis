...
# Change to dplyr
find_degree <- function(num.nodes, all.nodes) {
  degree.list <- as.vector(rep(0, nrow(nodes)))
  for (i in all.nodes) {
    degree.list[i] <- degree.list[i] + 1
  }
  degree.list
}
...
# Divide color based on portfolio
colrs <- c("gray50", "tomato", "seashell", "thistle", "slateblue")
V(net)$color <- colrs[V(net)$portfolio]

# Sets node size relative to its degree
all <- c(edges$from, edges$to)
degs <- find_degree(nrow(nodes), all)
V(net)$size <- degs
...
# Animation 1: Showcases different layouts
net.js <- net
layouts <- graphjs(net.js, 
                     layout = list(
                       layout_randomly(net.js, dim = 3),
...
# Click animation 2: Cumulative expandable
click2 <- Map(function(i)
{
  # Expand layout for new nodes
  layout <- l0
  layout[V(net)$portfolio == i, ] <- l1[V(net)$portfolio == i, ]
  ...
  # Update graph
  list(layout = layout, vertex.color = c, cumulative = TRUE)
  
}, seq(idx))
names(click2) <- paste(idx)

net.js <- net
click.ani2 <- (graphjs(net.js,
                       layout = l0,
                       click = click2,
                       vertex.color = col,
                       vertex.label = V(net)$label,
...
saveWidget(click.ani2, file = "Order-Network-Click2.html")
browseURL("Order-Network-Click2.html")
